import type { NDVector, ProjectionResult } from './types.js';
...
export class Projection {
  ...
  constructor(n: number) {
    if (n < 3) throw new RangeError(`Projection needs n ≥ 3, got ${n}`);
    this.n = n;
  ...
  setDistance(sourceDim: number, value: number): void {
  ...
  getDistance(sourceDim: number): number {
    if (sourceDim < 4 || sourceDim > this.n) {
      throw new RangeError(`sourceDim ${sourceDim} out of range [4, ${this.n}]`);
    }
   ...
   */
  projectVertex(v_cam: NDVector): ProjectionResult {
    const n = this.n;
    ...
    while (curDim > 3) {
      ...
      if (Number.isFinite(d)) {
        ...
        const k = d / margin;
        for (let i = 0; i < curDim - 1; i++) v[i] *= k;
      }
   ...
   * рёбер 1-куба).
   */
  projectEdge(v1_cam: NDVector, v2_cam: NDVector): EdgeProjectionResult {
    const n = this.n;
    ...
    while (curDim > 3) {
      ...
      if (Number.isFinite(d)) {
        ...
        if (minM < minMargin) minMargin = minM;
        ...
        if (behind1 !== behind2) {
          ...
          if (t > 0 && t < 1) {
            ...
            const inplaceLerp = (src: Float64Array, dst: Float64Array, alpha: number): void => {
...
export interface EdgeProjectionResult {
  /** null — ребро полностью отсечено. */
  result: null | {
    pos1: [number, number, number];
    pos2: [number, number, number];
    /** Глубины (значения w на каждом уровне) для конца 1 — длина n - 3. */
    hiddenDepths1: number[];
    hiddenDepths2: number[];
    /** min(d_i + w_i) по всем уровням и обеим вершинам. Маленькое → event horizon. */
    minMargin: number;
    /** Доля ребра, отрезанная при клиппинге (0 = ничего, ≈1 = почти всё). */

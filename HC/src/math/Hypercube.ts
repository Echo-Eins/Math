import type { NDVector, VertexId, Edge, Face } from './types.js';
...
export class Hypercube {
  ...
  private readonly vertexCoords: NDVector[];
   ...
   */
  readonly edgesByAxis: ReadonlyArray<ReadonlyArray<Edge>>;

  /**
   * Список соседей каждой вершины (n штук на вершину).
   * neighbors[v][i] = v ⊕ (1 << i) — сосед по i-й оси.
   */
  readonly neighbors: ReadonlyArray<ReadonlyArray<VertexId>>;
  ...
  constructor(n: number) {
    ...
    const edges: Edge[] = [];
    const edgesByAxis: Edge[][] = Array.from({ length: n }, () => []);
    for (let v = 0; v < this.vertexCount; v++) {
      for (let i = 0; i < n; i++) {
        if (((v >> i) & 1) === 0) {
          const e: Edge = { a: v, b: v | (1 << i), axis: i };
          edges.push(e);
    ...
    // Соседи
    const neighbors: VertexId[][] = new Array(this.vertexCount);
    for (let v = 0; v < this.vertexCount; v++) {
      const row = new Array<VertexId>(n);
      for (let i = 0; i < n; i++) row[i] = v ^ (1 << i);
      neighbors[v] = row;
    }
  ...
  coords(v: VertexId): NDVector {
    return this.vertexCoords[v];
  }

  /** Копия координат вершины. */
  coordsCopy(v: VertexId): NDVector {
   ...
   */
  faceVertices(face: Face): VertexId[] {
   ...
   * x₀ и x₂ свободны.
   */
  buildFace(fixed: Record<number, -1 | 1>): Face {
 ...
 */
export function* subsetsOf(mask: number): Generator<number> {
  ...
  while (true) {
    ...
    if (subset === mask) break; // защита от зацикливания (не должна срабатывать)
    if (subset === 0) {
 ...
 */
export function* subsetsOfSize(n: number, k: number): Generator<number> {
  if (k < 0 || k > n) return;
  if (k === 0) {
    yield 0;
    return;

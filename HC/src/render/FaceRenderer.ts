...
import type { Face, VertexId } from '../math/types.js';
...
export class FaceRenderer {
  ...
  private activeQuads = 0;

  constructor(_hypercube: Hypercube, maxQuads: number = 256) {
   ...
   */
  static orderedQuadVertices(face: Face, hypercube: Hypercube): VertexId[] {
    if (face.dim !== 2) {
    ...
    for (let i = 0; i < hypercube.n; i++) {
      if ((face.freeMask >> i) & 1) free.push(i);
    }
    if (free.length !== 2) throw new Error('Bad freeMask for 2-face');
    const [u, v] = free;
   ...
   */
  update(quads: ReadonlyArray<{
    positions: ReadonlyArray<[number, number, number]>; // длина 4
   ...
   * (зафиксированную в ±1).
   */
  static boundary2FacesOf3Cell(cell: Face, hypercube: Hypercube): Face[] {
    if (cell.dim !== 3) throw new Error(`expected 3-cell, got dim=${cell.dim}`);
    const freeAxes: number[] = [];
    for (let i = 0; i < hypercube.n; i++) {
      if ((cell.freeMask >> i) & 1) freeAxes.push(i);
    }
    if (freeAxes.length !== 3) throw new Error('Bad freeMask for 3-cell');
    const out: Face[] = [];

...
export class KnifeClip {
    ...
    static isVisible(face, camera) {
        ...
        for (let a = 0; a < n; a++) {
            if ((freeMask >> a) & 1)
                continue; // свободная ось — пропускаем
     ...
     */
    static filter(faces, camera) {
        const out = [];
        for (const f of faces) {
            if (KnifeClip.isVisible(f, camera))
                out.push(f);
        }
        return out;
    }
     ...
     */
    static cameraInsideCube(camera) {
        const n = camera.n;
        for (let i = 0; i < n; i++) {
            if (Math.abs(camera.position[i]) >= 1)
                return false;
        }
        return true;
    }

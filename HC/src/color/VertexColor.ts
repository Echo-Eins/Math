import type { NDVector } from '../math/types.js';
 ...
 */
export function vertexColor(coordsInCamera: NDVector): [number, number, number] {
  const n = coordsInCamera.length;
  const map = (x: number): number => 0.25 + 0.75 * (x * 0.5 + 0.5);
 ...
 */
export function applyEventHorizon(
  color: [number, number, number],
  margin: number,
  threshold: number
): [number, number, number] {
  if (!Number.isFinite(margin) || margin >= threshold) return color;
